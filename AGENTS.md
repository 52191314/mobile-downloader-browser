...
## Play Console release notes — official 15 languages
...
2 lines max each. The 15 official language tags used by the app:

```
<en-US>   English (US / Global)
<ar>      Arabic (bare "ar")
<de-DE>   German
...
- **Documentation Sources & Change Documentation**: Every fix, architectural tweak, or platform change must be grounded in official documentation sources (e.g., Android Developers documentation, Flutter API reference, Chromium/RFC specs). All changes must be explicitly documented in corresponding release notes (`RELEASE_NOTES_x.y.z.md`), commit descriptions, and architecture notes.
